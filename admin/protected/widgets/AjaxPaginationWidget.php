...
class AjaxPaginationWidget extends CLinkPager {
     ...
     */
     protected function createAjaxLink($page)
     {
        $url = $this->createPageUrl($page);
        list($url,$params) = explode('?',$url);
        $c = $this->getController();
        $params = $c->getActionParams();
        $params['page'] = $page+1;
        $params = json_encode($params);
        $url = sprintf("javascript:pagination('%s',%s,'%s')",$url,$params,$this->selector);
        return $url;
    ...
    protected function createPageButton($label, $page, $class, $button, $selected) {
        ...
        if ($button) {
            ...
            return '<li class="' . $class . '">' . CHtml::link($image, $this->createAjaxLink($page), array("class" => "rollover")) . '</li>';
        }

        if ($page == ($firstPage + 1) && $currentPage > ($firstPage + 3) && $this->getPageCount() > 7) {
            //return '<li>' . CHtml::link($label, $this->createAjaxLink($page)) . '<span class="skip-marker">…</span></li>';
												return '<li>' . CHtml::link($label, $this->createAjaxLink($page)) . '</li><li><span class="skip-marker">...</span></li>';
        }

        if ($page == ($lastPage - 2) && ($currentPage < $lastPage - 4) && $this->getPageCount() > 7) {
            //return '<li><span class="skip-marker">…</span>' . CHtml::link($label, $this->createAjaxLink($page)) . '</li>';
												return '<li><span class="skip-marker">...</span></li><li>' . CHtml::link($label, $this->createAjaxLink($page)) . '</li>';
        }
        ...
        return '<li>' . CHtml::link($label,$this->createAjaxLink($page)) . '</li>';
    }
